...
export default defineEventHandler(async (event) => {
  ...
  if (offre.status === 'ARCHIVED') {
    throw createError({ statusCode: 400, statusMessage: 'Impossible de changer le statut d\'une offre archivée' })
  }

  const newStatus = offre.status === 'ACTIVE' ? 'INACTIVE' : 'ACTIVE'
